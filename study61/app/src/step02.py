...
def collector_node(state: MessagesState) -> Command[Literal["supervisor"]]:
    ...
    messages = [{"role": "system", "content": "당신은 **데이터 수집** 전문가입니다. 오직 데이터 수집만 진행 후 답변을 제공해야 합니다."}] + state["messages"]
...
def analyst_node(state: MessagesState) -> Command[Literal["supervisor"]]:
    ...
    messages = [{"role": "system", "content": "당신은 **데이터 분석** 전문가입니다. 오직 데이터 분석만 진행 후 답변을 제공해야 합니다."}] + state["messages"]
    response = llm.invoke(messages)
    return Command(update={"messages": [response]}, goto=SUPERVISOR)
...
def supervisor_node(state: MessagesState) -> Command[Literal["collector", "analyst", "__end__"]]:
    ...
    if "FINISH" in decision.upper():
        ...
        # 모호할 경우 종료하거나 기본값 지정
        print(f"❗ Supervisor의 결정이 모호합니다: '{decision}'. 작업을 종료합니다.")
        return Command(goto=END)
...
def run():
  ...
  for event in graph.stream(inputs, config, stream_mode="values"):
    if "messages" in event:
      ...
      logger.info(f"[{last_msg.type}]: {last_msg.content}...")
