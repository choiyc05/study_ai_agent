      ...
      <div className="p-3 border-top bg-white">
        <div className="container" style={{ maxWidth: '700px' }}>
          <div className="input-group border rounded-pill px-3 py-1 shadow-sm">
            <input type="text" className="form-control border-0 shadow-none" placeholder="메시지를 입력하세요..." value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && Send()} />
            <button className="btn btn-link text-primary fw-bold text-decoration-none" onClick={Send}> 전송 </button>
          </div>
        </div>
      </div>
    </div>
